...
def dashboard(r):
    data ={}
    data['studentCount']  =Student.objects.filter(isApproved=True).count()
    data['admissionCount']  =Student.objects.filter(isApproved=False).count()
    data['paymentCount']=Payment.objects.all().count()
